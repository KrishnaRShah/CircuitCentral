...
export const addOrderToCustomer = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`No customer with id: ${id}`);
  const updatedCustomer = await Customer.findByIdAndUpdate(id, { $push: { orders: order } }, { new: true });
  res.json(updatedCustomer);
}

export const deleteOrderFromCustomer = async (req, res) => {
  const { id } = req.params;
  const { order } = req.body;
  if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`No customer with id: ${id}`);
  const updatedCustomer = await Customer.findByIdAndUpdate(id, { $pull: { orders: { _id: order._id } } }, { new: true });
  res.json(updatedCustomer);
}
